...
export default function ProFieldTools(){
  ...
  const arch=useMemo(()=>{
    const c=n(span),h=n(rise);if(c<=0||h<=0)return null;
  ...
  return <div className="fieldTools">
    ...
    <details className="toolDrawer">
      ...
      <div className="calcGrid archCalc">
        ...
        <label>Arch rise (in)<input value={rise} onChange={e=>setRise(e.target.value)} inputMode="decimal"/></label>
      </div>
    ...
    <details className="toolDrawer">
      ...
      <div className="calcGrid hearthCalc">
        ...
        <label>Opening width (in)<input value={openingW} onChange={e=>setOpeningW(e.target.value)} inputMode="decimal"/></label>
        <label>Hearth depth from opening plane (in)<input value={hearthD} onChange={e=>setHearthD(e.target.value)} inputMode="decimal"/></label>
      </div>
